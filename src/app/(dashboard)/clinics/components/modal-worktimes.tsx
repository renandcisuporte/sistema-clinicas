...
import { useHandleCalculateTotalHours } from '@/hooks/use-work-times'
      ...
      >
        <ScrollArea className="h-full">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 p-6">
            ...
            {fields.map((field, i) => {
              const times = watch(`worktimes.${i}.times`)
              const open = watch(`worktimes.${i}.open`)
              const week = watch(`worktimes.${i}.week`)

              return (
                <div key={field.id}>
                  <input type="hidden" {...register(`worktimes.${i}.week`)} />
                  ...
                  <h4 className="flex items-center justify-between space-x-2">
                    <span className="flex items-center space-x-2 font-semibold">
                      ...
                      <Plus
                        ...
                        onClick={() => append({ week, open, times })}
                      />
                    </span>
                    ...
                    {field.times.map((__, ii) => (
                      ...
                      >
                        <input
                          ...
                          defaultValue={'oi'}
                          {...register(
                            `worktimes.${i}.times.${ii}.description`,
                          )}
                        />
                        ...
                        */}
                        ...
                        <Controller
                          control={control}
                          name={`worktimes.${i}.times.${ii}.time`}
                          render={({ field }) => (
                            <InputLabel
                              label={field.name}
                              type="time"
